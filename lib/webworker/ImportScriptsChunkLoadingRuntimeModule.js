...
class ImportScriptsChunkLoadingRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		const baseURI = RuntimeGlobals.baseURI;
		const afterSelfLocation = JSON.stringify(
		...
		console.log(
			JSON.stringify({
				baseURI,
				afterSelfLocation,
				chunkScriptFileName: RuntimeGlobals.getChunkScriptFilename
			})
		);
		return Template.asString([
			withBaseURI
				? Template.asString([
						`${baseURI} = self.location + ${afterSelfLocation};`
				  ])
				: "// no baseURI",
			"",
								...
								? [
										...
										Template.indent([
											...
											Template.indent(
												`importScripts(${
													withCreateScriptUrl
														? `${RuntimeGlobals.createScriptUrl}(${RuntimeGlobals.getChunkScriptFilename}(chunkId))`
			...
			withHmr
				? Template.asString([
						...
						Template.indent([
							...
							`importScripts(${
								withCreateScriptUrl
									? `${RuntimeGlobals.createScriptUrl}(${RuntimeGlobals.getChunkUpdateScriptFilename}(chunkId))`
