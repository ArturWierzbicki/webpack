...
class AutoPublicPathRuntimeModule extends RuntimeModule {
	...
	generate() {
		...
		const overrideAutoPublicPath = Template.asString([
			`if (${RuntimeGlobals.global}.__webpack_override_auto_public_path__)`,
			Template.indent(
				`scriptUrl = ${RuntimeGlobals.global}.__webpack_override_auto_public_path__;`
			)
		]);
		...
		return Template.asString([
			...
			scriptType === "module"
				? Template.asString([
						overrideAutoPublicPath,
						`else if (typeof ${importMetaName}.url === "string") scriptUrl = ${importMetaName}.url`
				  ])
				: Template.asString([
						overrideAutoPublicPath,
						"else {",
